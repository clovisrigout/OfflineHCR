...
def main():
 ...
 # see https://www.tensorflow.org/tutorials/layers#input_layer
	...
	def getPredictions(X):
		...
		return predictions["probabilities"].eval(feed_dict={x: X, keep_prob: 1})
	...
	with tf.Session() as sess:
		...
		if(TRAIN_MODEL):
			...
			# TEST LABELS MATCH WORDS
			for word in data.words:
				imageD = [e[0] for e in word]
				labels = [e[1] for e in word]
				wordLabel = "".join(labels)
				if not os.path.exists("../data/test/{}".format(wordLabel)):
					os.maximizekedirs("../data/test/{}".format(wordLabel))
				for d, label in zip(imageD, labels):
					count = 0
					im = Image.fromarray(d)
					im.save("../data/test/{}/{}_{}.png".format(wordLabel,label,count))
					count += 1
			...
			if(WITH_LM):
				...
				if(WITH_DICTIONARY):
					...
					# maximize likelihood over all words:
					wordPreds = []
					wordTrues = []
					for word, preds in zip(data.words, predictions):
						labels = [e[1] for e in word]
...
def getAllLengthKWords(k):
	lengthKWords = [word.split("-")[0].lower() for word in englishWords.words() if len(word) == k]
	if k == 6:
